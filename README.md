# Sam collection of WAMS
A collection of WAMS hosted by Jempasam.
Used by the app "WAMJamParty".
Mostly generated using FaustIDE.

Test host [here](https://jempasam.github.io/SamWAMS/)

# All the WAMS

|    | module\filter\arpeggio_shifter                                                                      |
|----|-----------------------------------------------------------------------------------------------------|
|Name| arpeggio_shifter 1.0                                                                                |
|Desc| A pitch shifter based on an arpeggiator.                                                            |
|Use | https://jempasam.github.io/SamWAMS/module/filter/arpeggio_shifter/index.js                          |
|Test| https://jempasam.github.io/SamWAMS/#module/filter/arpeggio_shifter                                  |

|    | module\filter\bandpass                                                                              |
|----|-----------------------------------------------------------------------------------------------------|
|Name| thunder 1.0.0                                                                                       |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/filter/bandpass/index.js                                  |
|Test| https://jempasam.github.io/SamWAMS/#module/filter/bandpass                                          |

|    | module\filter\bandpass_resonant                                                                     |
|----|-----------------------------------------------------------------------------------------------------|
|Name| bandpass 1.0.0                                                                                      |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/filter/bandpass_resonant/index.js                         |
|Test| https://jempasam.github.io/SamWAMS/#module/filter/bandpass_resonant                                 |

|    | module\filter\bit_crusher                                                                           |
|----|-----------------------------------------------------------------------------------------------------|
|Name| bit_crusher 1.0                                                                                     |
|Desc| A simple bit crusher effect.                                                                        |
|Use | https://jempasam.github.io/SamWAMS/module/filter/bit_crusher/index.js                               |
|Test| https://jempasam.github.io/SamWAMS/#module/filter/bit_crusher                                       |

|    | module\filter\continuous_midi_filter                                                                |
|----|-----------------------------------------------------------------------------------------------------|
|Name| continuous_midi_filter 1.0                                                                          |
|Desc| A filter that contiously adjust pitch of an audio input depending on his last midi inputs. With only one voice at a time.|
|Use | https://jempasam.github.io/SamWAMS/module/filter/continuous_midi_filter/index.js                    |
|Test| https://jempasam.github.io/SamWAMS/#module/filter/continuous_midi_filter                            |

|    | module\filter\glitcher                                                                              |
|----|-----------------------------------------------------------------------------------------------------|
|Name| glitcher 1.0                                                                                        |
|Desc| Make the sound glitch.                                                                              |
|Use | https://jempasam.github.io/SamWAMS/module/filter/glitcher/index.js                                  |
|Test| https://jempasam.github.io/SamWAMS/#module/filter/glitcher                                          |

|    | module\filter\large_arpeggio_shifter                                                                |
|----|-----------------------------------------------------------------------------------------------------|
|Name| large_arpeggio_shifter 1.0                                                                          |
|Desc| A large pitch shifter based on an arpeggiator.                                                      |
|Use | https://jempasam.github.io/SamWAMS/module/filter/large_arpeggio_shifter/index.js                    |
|Test| https://jempasam.github.io/SamWAMS/#module/filter/large_arpeggio_shifter                            |

|    | module\filter\megamouth                                                                             |
|----|-----------------------------------------------------------------------------------------------------|
|Name| megamouth 1.0                                                                                       |
|Desc| Simulates sound waves in the human vocal tract to make vowel sounds.                                |
|Use | https://jempasam.github.io/SamWAMS/module/filter/megamouth/index.js                                 |
|Test| https://jempasam.github.io/SamWAMS/#module/filter/megamouth                                         |

|    | module\filter\midi_filter                                                                           |
|----|-----------------------------------------------------------------------------------------------------|
|Name| midi_filter 1.0                                                                                     |
|Desc| A filter that adjust pitch and gain of an audio input depending on his midi inputs.                 |
|Use | https://jempasam.github.io/SamWAMS/module/filter/midi_filter/index.js                               |
|Test| https://jempasam.github.io/SamWAMS/#module/filter/midi_filter                                       |

|    | module\filter\pitchshifter                                                                          |
|----|-----------------------------------------------------------------------------------------------------|
|Name| pitchshifter 1.0.0                                                                                  |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/filter/pitchshifter/index.js                              |
|Test| https://jempasam.github.io/SamWAMS/#module/filter/pitchshifter                                      |

|    | module\filter\vibrato                                                                               |
|----|-----------------------------------------------------------------------------------------------------|
|Name| bandpass 1.0.0                                                                                      |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/filter/vibrato/index.js                                   |
|Test| https://jempasam.github.io/SamWAMS/#module/filter/vibrato                                           |

|    | module\generator\alarm                                                                              |
|----|-----------------------------------------------------------------------------------------------------|
|Name| alarm 0.0                                                                                           |
|Desc| Produce an alarm-like sound                                                                         |
|Use | https://jempasam.github.io/SamWAMS/module/generator/alarm/index.js                                  |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/alarm                                          |

|    | module\generator\bird                                                                               |
|----|-----------------------------------------------------------------------------------------------------|
|Name| bird 1.0.0                                                                                          |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/generator/bird/index.js                                   |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/bird                                           |

|    | module\generator\endless_risset                                                                     |
|----|-----------------------------------------------------------------------------------------------------|
|Name| midi_filter 1.0                                                                                     |
|Desc| An infinite glissando generator.                                                                    |
|Use | https://jempasam.github.io/SamWAMS/module/generator/endless_risset/index.js                         |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/endless_risset                                 |

|    | module\generator\fire                                                                               |
|----|-----------------------------------------------------------------------------------------------------|
|Name| fire 1.0.0                                                                                          |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/generator/fire/index.js                                   |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/fire                                           |

|    | module\generator\fm3                                                                                |
|----|-----------------------------------------------------------------------------------------------------|
|Name| fm3 1.0                                                                                             |
|Desc| An oscillator modulating an oscillator modulating an oscillator                                     |
|Use | https://jempasam.github.io/SamWAMS/module/generator/fm3/index.js                                    |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/fm3                                            |

|    | module\generator\impulsion_resonator                                                                |
|----|-----------------------------------------------------------------------------------------------------|
|Name| impulsion_resonator 1.0                                                                             |
|Desc| A resonance and impulsion based sound generator.                                                    |
|Use | https://jempasam.github.io/SamWAMS/module/generator/impulsion_resonator/index.js                    |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/impulsion_resonator                            |

|    | module\generator\insects                                                                            |
|----|-----------------------------------------------------------------------------------------------------|
|Name| insects 1.0.0                                                                                       |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/generator/insects/index.js                                |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/insects                                        |

|    | module\generator\noise_resonator                                                                    |
|----|-----------------------------------------------------------------------------------------------------|
|Name| noise_resonator 1.0                                                                                 |
|Desc| A resonance and noise based sound generator.                                                        |
|Use | https://jempasam.github.io/SamWAMS/module/generator/noise_resonator/index.js                        |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/noise_resonator                                |

|    | module\generator\occillator                                                                         |
|----|-----------------------------------------------------------------------------------------------------|
|Name| occilator 1.0.0                                                                                     |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/generator/occillator/index.js                             |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/occillator                                     |

|    | module\generator\rain                                                                               |
|----|-----------------------------------------------------------------------------------------------------|
|Name| complex_rain 1.0.0                                                                                  |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/generator/rain/index.js                                   |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/rain                                           |

|    | module\generator\supersaw                                                                           |
|----|-----------------------------------------------------------------------------------------------------|
|Name| supersaw 0.0                                                                                        |
|Desc| 7 sawtooth overlapped, with a detune relative to the previous sawtooth.                             |
|Use | https://jempasam.github.io/SamWAMS/module/generator/supersaw/index.js                               |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/supersaw                                       |

|    | module\generator\thunder                                                                            |
|----|-----------------------------------------------------------------------------------------------------|
|Name| thunder 1.0.0                                                                                       |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/generator/thunder/index.js                                |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/thunder                                        |

|    | module\generator\violon                                                                             |
|----|-----------------------------------------------------------------------------------------------------|
|Name| violon 1.0.0                                                                                        |
|Desc| Simple violin physical model controlled with continuous parameters.                                 |
|Use | https://jempasam.github.io/SamWAMS/module/generator/violon/index.js                                 |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/violon                                         |

|    | module\generator\voice                                                                              |
|----|-----------------------------------------------------------------------------------------------------|
|Name| voice 1.0.0                                                                                         |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/generator/voice/index.js                                  |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/voice                                          |

|    | module\generator\wind                                                                               |
|----|-----------------------------------------------------------------------------------------------------|
|Name| wind 1.0.0                                                                                          |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/generator/wind/index.js                                   |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/wind                                           |

|    | module\generator\windchimes                                                                         |
|----|-----------------------------------------------------------------------------------------------------|
|Name| windchimes 1.0.0                                                                                    |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/module/generator/windchimes/index.js                             |
|Test| https://jempasam.github.io/SamWAMS/#module/generator/windchimes                                     |

|    | simple\effect\flanger                                                                               |
|----|-----------------------------------------------------------------------------------------------------|
|Name| Flanger 1.0.0                                                                                       |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/simple/effect/flanger/index.js                                   |
|Test| https://jempasam.github.io/SamWAMS/#simple/effect/flanger                                           |

|    | simple\effect\graingenerator                                                                        |
|----|-----------------------------------------------------------------------------------------------------|
|Name| Grain Generator 1.0.0                                                                               |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/simple/effect/graingenerator/index.js                            |
|Test| https://jempasam.github.io/SamWAMS/#simple/effect/graingenerator                                    |

|    | simple\effect\granular_reverse                                                                      |
|----|-----------------------------------------------------------------------------------------------------|
|Name| Reverse Grain 0.0                                                                                   |
|Desc| A granular effect with reversed grain playback                                                      |
|Use | https://jempasam.github.io/SamWAMS/simple/effect/granular_reverse/index.js                          |
|Test| https://jempasam.github.io/SamWAMS/#simple/effect/granular_reverse                                  |

|    | simple\instrument\fmsynth                                                                           |
|----|-----------------------------------------------------------------------------------------------------|
|Name| fmfx 1.0.0                                                                                          |
|Desc| A FM FX Synthesizer with reverb and flanger effects                                                 |
|Use | https://jempasam.github.io/SamWAMS/simple/instrument/fmsynth/index.js                               |
|Test| https://jempasam.github.io/SamWAMS/#simple/instrument/fmsynth                                       |

|    | simple\instrument\frenchbell                                                                        |
|----|-----------------------------------------------------------------------------------------------------|
|Name| frenchbell 1.0.0                                                                                    |
|Desc| French church bell physical model.                                                                  |
|Use | https://jempasam.github.io/SamWAMS/simple/instrument/frenchbell/index.js                            |
|Test| https://jempasam.github.io/SamWAMS/#simple/instrument/frenchbell                                    |

|    | simple\instrument\organ                                                                             |
|----|-----------------------------------------------------------------------------------------------------|
|Name| organ 1.0.0                                                                                         |
|Desc| Organ physical model.                                                                               |
|Use | https://jempasam.github.io/SamWAMS/simple/instrument/organ/index.js                                 |
|Test| https://jempasam.github.io/SamWAMS/#simple/instrument/organ                                         |

|    | simple\instrument\paradigma9                                                                        |
|----|-----------------------------------------------------------------------------------------------------|
|Name| paradigma9 0.0.7                                                                                    |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/simple/instrument/paradigma9/index.js                            |
|Test| https://jempasam.github.io/SamWAMS/#simple/instrument/paradigma9                                    |

|    | simple\instrument\superclav                                                                         |
|----|-----------------------------------------------------------------------------------------------------|
|Name| superclav 1.0.0                                                                                     |
|Desc|                                                                                                     |
|Use | https://jempasam.github.io/SamWAMS/simple/instrument/superclav/index.js                             |
|Test| https://jempasam.github.io/SamWAMS/#simple/instrument/superclav                                     |

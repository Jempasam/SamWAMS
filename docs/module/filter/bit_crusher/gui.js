import { FaustUI } from '../../../common/faust-ui/index.js';
...
class FaustDefaultGui extends HTMLElement {
	...
	constructor(wamNode, faustNode, style) {
		...
		$style.innerHTML = style.replaceAll("rgba(80, 80, 80, 0.75)", "#5937B7");
...
const createElement = async (plugin) => {
	...
	const style = await (await fetch(new URL('./../../../common/faust-ui/index.css', import.meta.url).href)).text();
